...
(function(root){
  ...
  function select({query='',restaurant='',collection='copycat',settings=null,match=false}={}){
    ...
    return entries.filter(m=>{const r=E.recipeMap[m.recipe],text=normalize(`${m.item} ${m.restaurant} ${r.name} ${r.parts.map(p=>D.ingredients[p.id].name).join(' ')}`);return (!restaurant||m.restaurant===restaurant)&&terms.every(t=>text.includes(t))&&(!match||E.allowed(r,settings));});
  }
  function render(entries,{prices={},equivalent=null,link=false}={}){
    return entries.map(m=>{const r=E.recipeMap[m.recipe],n=E.nutrition(r,1,prices),action=link?`<a class="button subtle small menu-action" href="../?recipe=${encodeURIComponent(r.id)}">Compare make or buy ↗</a>`:`<button class="button subtle small menu-action" data-recipe="${r.id}">Compare make or buy ↗</button>`;
